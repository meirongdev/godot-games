...
| `session.token_expiry_sec` | **≥ 3600**(Nakama 默认只有 **60**) | ⚠️ 见下面的「为什么这条是硬要求」。默认值会让大厅在登录一分钟后失效 |
| `session.refresh_token_expiry_sec` | 建议 **≥ 86400**(默认 3600) | 页面能在后台放多久还救得回来。过了它客户端只能提示「刷新页面重新进来」—— 家里人常常把页面开着放一天 |
...
#### 3.2.1 为什么 `session.token_expiry_sec` 是硬要求

**2026-08-27 上线后「手机无法进入房间」的根因就是这一条没写在契约里。**

Nakama 的 `session.token_expiry_sec` 默认是 **60 秒**(实测:不带这个参数起一个
3.40.0,签出来的 token `exp - iat = 60`)。而本仓库的 `nakama/docker-compose.yml`
显式写了 `--session.token_expiry_sec 7200` —— 于是本地怎么玩都正常,线上一分钟后废。

一分钟后具体坏成什么样(实测,客户端修好之前):

| 现象 | 为什么 |
|---|---|
| 大厅里现有房间**全部消失**,显示「还没有房间 —— 建一个吧」 | `list_rooms` RPC 401,客户端拿不到列表就渲染成空 |
| 点建房报一句英文 `Auth token invalid` | `create_room` RPC 401 |
| **聊天和「在线」列表却完全正常** | 那两个走 socket,而 token 过期**不会**关掉已经建立的 socket(实测) |
| 而且永远不会自己恢复 | socket 没断 → 客户端不会重连 → 没有任何路径去刷 session |

客户端侧已经补上了(每个 RPC 前按需刷 session,重连前也刷一次),所以现在就算
部署侧用默认值也能玩。**但仍然要求 ≥3600**:60 秒的 token 意味着每个客户端每
30 秒就多一次 refresh 往返,纯属白烧;而且这条链路上任何一处出问题,症状就是
「大厅看不到房间」—— 那是个很难往「token 有效期」上联想的现象。

> 判别法:线上开着页面**在大厅坐两分钟**,房间列表还在、还能建房,这条就是对的。
...
- **客户端会自己重连,也会自己重新进房。** 手机锁屏 / 切后台时浏览器会冻结页面,
  而 Chrome 冻页面时**直接关掉 WebSocket**(实测:控制台报 "Page entered
  Back-Forward Cache" 紧跟一条 WS close)。客户端每 3 秒巡检一次,断了就刷
  session、重连、重新 `match_join`。所以网关上的 WebSocket 空闲超时不是灾难,
  但**必须允许重连**(别按 IP/会话做粘性限流)。
- **没人的房间 60 秒后由服务端自动关掉**(`room.lua` 的空置计时)。一个人待在
  房间里锁屏超过一分钟,回来时房间已经不存在了 —— 客户端会说「房间已经关了」
  并退回大厅。这是设计行为,不是故障。
...
> ⚠️ **e2e 全绿 ≠ 网页能玩。** 这两个工具**直连 Nakama 域名的 443**,完全绕过
> §3.3.1 那两条路由 —— 路由写错、漏写、WebSocket 没放 Upgrade,e2e 一样全绿。
> 路由是 e2e 结构上覆盖不到的唯一一块。

#### 4.2.1 必须单独做的一步:真的开一次页面

这一步不能省,也没有任何自动检查能替代它。浏览器是唯一同时经过
「§3.3.1 的路由」和「客户端渲染」的路径:

1. 开 `https://<web 域名>/?player=<任意串>`
2. **看得见中文**。全是方块/空白 = 字体没进制品(见 §4.3)。
3. **`✊ ✋ ✌` 画得出来**(进一局猜拳)。这几个字形在 emoji 回退字体里。
4. 开浏览器 network 面板,确认:
   - `POST /v2/account/authenticate/device` → **200**(路由 `/v2/*` 通了)
   - `/ws` → **101 Switching Protocols**(WebSocket 升级放行了)
   - 两者的域名都是**页面自己的域名**,不是 nakama 的域名

2026-08-27 的教训:整套中文字体缺失躲过了 82 项单测、场景检查、4 项 e2e、
